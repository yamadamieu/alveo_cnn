#ifndef DNNKERNEL_CONV1_H
#define DNNKERNEL_CONV1_H
#include <ap_fixed.h>
typedef ap_fixed<16, 7, AP_RND, AP_SAT> fixed;
namespace dnnk {


void conv1(const fixed* x, const fixed* weight, const fixed* bias, int width, int height,int in_channels, int out_channels, int ksize, fixed* y) {
    const int UNROLL_OCH = 32;
    const int UNROLL_X = 2;
